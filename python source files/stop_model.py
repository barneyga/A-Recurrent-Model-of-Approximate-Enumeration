...
class StopRecurrentAttention(nn.Module):
    ...
    def __init__(
        ...
        self.stopper = modules.StopNetwork(hidden_size, 1)
        self.baseliner = modules.BaselineNetwork(hidden_size, 1)

    def forward(self, x, l_t_prev, h_t_prev, s_t_prev, last=False):
        ...
        Args:
            ...
            s_t_prev: a 1D tensor of shape (B, 1). The stop
                signal vector for the previous timestep `t-1`.
        ...
        Returns:
            ...
            l_t: a 2D tensor of shape (B, 2). The location vector
                ...
                current timestep `t`.
            s_t: a 1D tensor of shape (B, 1). The stop vector
                containing the stop signal for the
                current timestep `t`.
            b_t: a vector of length (B,). The baseline for the
                current time step `t`.
            log_probas: a 2D tensor of shape (B, num_classes). The
                output log probability vector over the classes.
            l_log_pi: a vector of length (B,). The log probability of the 
            sampled next location l_t.
            s_log_pi: a vector of length (B,). The log probability of the 
            sampled stop signal s_t.
        """
        g_t = self.sensor(x, l_t_prev)
        h_t = self.rnn(g_t, h_t_prev)

        l_log_pi, l_t = self.locator(h_t)
        s_log_pi, s_t = self.stopper(h_t, s_t_prev, last)
        b_t = self.baseliner(h_t).squeeze()

        #if last:
        log_probas = self.classifier(h_t)
        return h_t, l_t, s_t, b_t, log_probas, l_log_pi, s_log_pi

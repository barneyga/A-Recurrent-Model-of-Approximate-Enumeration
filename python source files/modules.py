...
class StopNetwork(nn.Module):
    ...
    Uses the internal state `h_t` of the core network to
    produce the stop signals `s_t` for the next
    time step.

    Concretely, feeds the hidden state `h_t` through a fc
    layer followed by a sigmoid to clamp the output beween
    [0, 1]. This produces a 1D vector of means used to
    parametrize a bernoulli from which the stop signals `s_t`
    for the next time step are sampled.

    Hence, the stop signals `s_t` are chosen stochastically
    from a distribution conditioned on an affine
    transformation of the hidden state vector `h_t`.
    ...
    Returns:
        p: a 1D vector of shape (B, 1).
        s_t: a 1D vector of shape (B, 1).
    """
    ...
    def __init__(self, input_size, output_size):
        ...
        hid_size = input_size // 2
        self.fc = nn.Linear(input_size, hid_size)
        self.fc_st = nn.Linear(hid_size, output_size)

    def forward(self, h_t, s_t_prev, last):
        # compute mean
        feat = F.relu(self.fc(h_t.detach()))
        p = torch.sigmoid(self.fc_st(feat))

        # reparametrization trick
        s_t = torch.distributions.Bernoulli(p).sample()*s_t_prev
        s_t = s_t.detach()
        if last:
          s_t = torch.zeros_like(s_t)
        log_pi = Bernoulli(p).log_prob(s_t).squeeze(1)
